...
export default function KlafimIndexPage() {
  ...
  return (
    <div dir="rtl" style={{ minHeight: '100vh', background: '#f5f5f5', fontFamily: 'Heebo, Arial, sans-serif' }}>
      {/* Header */}
      <div style={{ background: 'var(--ys-heading)', borderBottom: '3px solid var(--ys-accent)', padding: '18px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        <div>
          <div style={{ fontSize: 20, fontWeight: 900, color: 'var(--ys-accent)' }}>📜 ניהול קלפים</div>
              ...
              >
                <div style={{
                  ...
                  <div style={{ padding: '12px 14px' }}>
                    <div style={{ fontWeight: 800, fontSize: 13, color: 'var(--ys-heading)', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {p.name}
                    </div>
                    <div style={{ fontSize: 11, color: '#888' }}>{p.cat ?? p.category}</div>
                    <div style={{
                      marginTop: 10, background: 'var(--ys-heading)', color: 'var(--ys-accent)',
