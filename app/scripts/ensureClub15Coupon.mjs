...
async function run() {
  ...
  if (snap.empty) {
    ...
    const needsUpdate = !data.active || data.discount !== 15;
    if (needsUpdate) {
      await docSnap.ref.update({ active: true, discount: 15 });
      console.log(`✏️  קופון עודכן: active=true, discount=15 (id: ${docSnap.id})`);
    } else {
      console.log(`✓ קופון קיים ותקין: ${CODE} (id: ${docSnap.id})`);
    }
    console.log('מצב סופי:', { ...data, active: true, discount: 15 });
  }
}
