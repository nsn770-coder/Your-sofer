...
export async function POST(req: NextRequest) {
  try {
    ...
    const itemsHtml = cert.items.map(item => `
      <tr>
        <td style="padding:8px 12px;border-bottom:1px solid #f0ece0;font-family:monospace;font-weight:700;color:#3B3B41;white-space:nowrap;">${item.serialNumber}</td>
...
<body style="margin:0;padding:0;background:#f5f0e8;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.1);">
    ...
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#3B3B41 0%,#1E40AF 100%);padding:28px 32px;text-align:center;border-bottom:3px solid #C5A028;">
    ...
    <div style="padding:32px;">
      <p style="font-size:16px;color:#3B3B41;font-weight:700;margin:0 0 6px;">שלום ${cert.customerName},</p>
      ...
      <div style="margin-bottom:24px;">
        <div style="font-size:12px;font-weight:700;color:#3B3B41;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.1em;">פריטים מאושרים</div>
      ...
      <div style="text-align:center;margin-bottom:24px;">
        <a href="${certUrl}" style="display:inline-block;background:#C5A028;color:#3B3B41;text-decoration:none;border-radius:10px;padding:13px 32px;font-weight:900;font-size:15px;margin-left:8px;">
          📜 צפייה בתעודה המלאה
        </a>
      </div>
      ...
      <div style="background:#f8f6f2;border-radius:8px;padding:12px 16px;text-align:center;">
        ...
        <div style="font-family:monospace;font-size:15px;font-weight:900;color:#3B3B41;letter-spacing:0.06em;">${certId}</div>
      </div>
    </div>
