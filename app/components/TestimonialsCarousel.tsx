      ...
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        <div key={testIdx} style={{ background: '#fff', borderRadius: 0, boxShadow: '0 4px 32px rgba(0,0,0,0.07)', padding: isMobile ? '36px 24px' : '48px 56px', display: 'flex', alignItems: 'flex-start', gap: 32, flexDirection: isMobile ? 'column' : 'row', animation: 'testFadeIn 0.55s ease' }}>
          <div style={{ flexShrink: 0, alignSelf: isMobile ? 'center' : 'flex-start' }}>
            {t.imageUrl ? (
              <img src={optimizeCloudinaryUrl(t.imageUrl, 200)} alt={t.name} width={84} height={84} loading="lazy" style={{ borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--ys-accent)' }} />
            ) : (
              <div style={{ width: 84, height: 84, borderRadius: '50%', background: 'var(--ys-dark-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '3px solid var(--ys-accent)' }}>
          ...
          <div style={{ flex: 1, textAlign: 'start' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end' }}>
              <span style={{ fontSize: 15, fontWeight: 900, color: 'var(--ys-text)' }}>{t.name}</span>
