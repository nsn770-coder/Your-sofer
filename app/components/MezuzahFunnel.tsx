...
function IconSun({ size = 36, color = '#1a1a1a' }: { size?: number; color?: string }) {
  return (
...
function IconHome({ size = 14, color = '#1a1a1a' }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function IconDoor({ size = 14, color = '#1a1a1a' }: { size?: number; color?: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M3 21h18" />
...
function IconGift({ size = 16, color = '#1a1a1a' }: { size?: number; color?: string }) {
  return (
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, backdropFilter: 'blur(6px)', background: 'rgba(0,0,0,0.55)' }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 0, width: '100%', maxWidth: 520, boxShadow: '0 32px 80px rgba(0,0,0,0.3)', overflow: 'hidden', direction: 'rtl', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ background: 'linear-gradient(135deg, #1a1a1a 0%, #1a1a1a 100%)', padding: '28px 24px', textAlign: 'center', position: 'relative' }}>
...
export default function MezuzahFunnel({ isMobile }: { isMobile: boolean }) {
  ...
  const locationConfig = [
    { key: 'room' as Location, label: 'חדר / פתח משני', sub: 'גדלים: 12 או 15 ס"מ', img: IMAGES.room, icon: <IconDoor size={14} color="#1a1a1a" /> },
    { key: 'entrance' as Location, label: 'כניסה ראשית', sub: 'גדלים: 15 או 20 ס"מ', img: IMAGES.entrance, icon: <IconHome size={14} color="#1a1a1a" /> },
  ];
  ...
  const nusachConfig: { key: Nusach; img?: string; fallbackIcon?: React.ReactNode; popular?: boolean }[] = [
    ...
    { key: 'תימני',  fallbackIcon: <IconSun size={40} color="#1a1a1a" /> },
  ];
                ...
                {!loading && klafim.length > 0 && (
                  <>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      ...
                      >
                        <IconArrowLeft size={14} color="#1a1a1a" /> לכל קלפי המזוזה
                      </button>
                    </div>
                  </>
                )}
