  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row sm:flex-col items-center sm:justify-center gap-3 p-4 sm:w-[40%] bg-gradient-to-b from-[#f8f6f2] to-white border-b sm:border-b-0 sm:border-l border-gray-100 flex-shrink-0">
        <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-full overflow-hidden bg-gray-100 flex-shrink-0 border-2 border-[#1a1a1a]/10 flex items-center justify-center">
          {sofer?.imageUrl ? (
        ...
        <div className="text-right sm:text-center sm:mt-2">
          {displayName ? (
            <>
              <p className="font-bold text-[#1a1a1a] text-sm leading-tight">{displayName}</p>
      ...
      <div className="flex flex-col flex-1">
        ...
        <div className="p-3 flex flex-col gap-2 flex-1">
          ...
          <div className="flex items-baseline gap-2 mt-auto">
            <span className="text-[18px] sm:text-lg font-black text-[#1a1a1a]">{formatPrice(price)}</span>
            {hasSale && (
              <>
          ...
          {qty === 0 ? (
            <button
              onClick={handleAdd}
              className="flex items-center justify-center gap-2 w-full py-[14px] sm:py-2 rounded-xl bg-[#1a1a1a] text-white text-[15px] sm:text-sm font-bold hover:bg-[#1a1a1a] transition-colors"
            >
              <IconCart size={13} />
              הוסף לסל
            </button>
            ...
            <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
              <button
                onClick={handleAdd}
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-[#1a1a1a] text-white text-sm font-bold hover:bg-[#1a1a1a] transition-colors"
              >
                <IconCheck size={10} />
                בסל ({qty})
              </button>
