...
function simulate({ prevSpent, prevPoints, total, shippingCost, cartItems }) {
  ...
  return { currentTier: currentTier.id, newTier: newTier.id, baseAmount, kippotBase, regularBase, pointsEarned, newTotalSpent, newPoints: prevPoints + pointsEarned };
}
...
for (const s of scenarios) {
  if (s.isGuest) {
    console.log(`\n${s.name}`);
    console.log('  → uid=null, אין משתמש — מדלג בשקט');
    continue;
  }
  const r = simulate(s);
  console.log(`\n${s.name}`);
  console.log(`  tier: ${r.currentTier} → ${r.newTier} | base: ₪${r.baseAmount} (kippot: ₪${r.kippotBase}, regular: ₪${r.regularBase})`);
  console.log(`  pointsEarned: ${r.pointsEarned} | newPoints: ${r.newPoints} | newTotalSpent: ₪${r.newTotalSpent}`);
}
